...
> #### Supplementary info
> The reference genome was obtained from [this](ftp://ftp.ensemblgenomes.org/pub/plants/release-39/fasta/brassica_oleracea/dna/Brassica_oleracea.v2.1.dna.toplevel.fa.gz) file, from which we retained only the
> FASTA records that correspond to a chromosome. These are the records matching the regular expression `/>C\d\s/`,
> any other records are scaffolds, which would match `>Scaffold\d+\s`.
